...
function queryServer(worker, message) {
  ...
  var handle = handles.get(key); // 第一次的时候是undefined, 第二次则在通过RoundRobinHandle创建server后，设置了值，下次的时候就不为空了，
  // 则就不执行RoundRobinHandle, 直接执行add, 把客户端句柄传递给worker进程，交由worker进程处理
  ...
  if (handle === undefined) {
    ...
    var constructor = RoundRobinHandle; // 这个里面通过net模块的createServer创建server,  并且进行监听
